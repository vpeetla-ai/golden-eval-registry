...
| `router_invariant` | `venkat-ai-platform`, `aegisai-enterprise-agent-platform`, `agent-finops`, `react-agent-pattern`, `swarm-agent-pattern`, `plan-execute-agent-pattern` | ✅ VAP orchestrator map · AegisAI gateway vocab/passport · FinOps outcome KPI vocab · **Acme embed panel break tests** (`acme.embed_invariant_v1`) · **ReAct bounded-loop safety** (`react_agent_pattern.bounded_loop_v1` — bounded vs unbounded `max_steps`, real trials against `ReActAgent`) · **Swarm fan-out vs serial** (`swarm_agent_pattern.fanout_v1` — coverage-per-round vs invocation-efficiency, real trials against `SwarmRuntime`) · **Plan-vs-reactive decomposition** (`plan_execute_agent_pattern.decomposition_v1` — decomposition accuracy + execution success, real trials against `PlanExecuteAgent`) |
...
| `critique_delta_gate` | `reflection-agent-pattern` | ✅ CI runs `reflection_agent.critique_delta_v1` — draft-only vs draft+critique-revised score delta, real trials against `ReflectionAgent` |
